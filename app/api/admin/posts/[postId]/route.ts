...
export async function DELETE(
  ...
  try {
    ...
    let reason = "Post removed by administrator";

    try {
      const body = await req.json();
      reason = String(body.reason || reason).trim() || reason;
    } catch {
      // no body provided for DELETE request
    }
    ...
    const now = new Date();
    const pointsDelta = getPostPointsDelta(post.status, "REMOVED");
    const idempotencyKey = req.headers.get("x-idempotency-key");
    let applied = false;

    await prisma.$transaction(async (tx) => {
      const removedResult = await tx.post.updateMany({
        where: {
          id: postId,
          status: {
            not: "REMOVED",
