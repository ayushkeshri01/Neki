...
export async function POST(
  ...
  try {
    ...
    let reason = "Post hidden by administrator";
    ...
    if (post.status === "REMOVED") {
      return NextResponse.json(
        { error: "Removed posts cannot be hidden" },
        { status: 400 }
      );
    }

    const now = new Date();
    const pointsDelta = getPostPointsDelta(post.status, "HIDDEN");
    const idempotencyKey = req.headers.get("x-idempotency-key");
    let applied = false;

    await prisma.$transaction(async (tx) => {
      const hiddenResult = await tx.post.updateMany({
        where: {
          id: postId,
          status: "VISIBLE",
        },
      ...
      await recordModerationEvent(tx, {
        ...
        notice: {
          ...
          noticeType: "POST_HIDDEN",
          title: "One of your posts was hidden",
          body: reason,
