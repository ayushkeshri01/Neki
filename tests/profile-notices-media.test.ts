...
process.env.DATABASE_URL ||= "postgresql://neki:nekineki@localhost:5432/neki";
process.env.AWS_REGION ||= "us-east-1";
process.env.AWS_S3_BUCKET ||= "neki-uploads";
...
test("S3 helpers sanitize file names and decode bucket keys safely", async () => {
  ...
  try {
    ...
    assert.equal(
      uploadedUrl,
      "https://neki-uploads.s3.us-east-1.amazonaws.com/uploads/1700000000000-my-avatar.png"
    );
    ...
    assert.equal(
      s3.getS3ObjectKey(
        "https://s3.us-east-1.amazonaws.com/neki-uploads/uploads/1700000000000-my-avatar.png"
      ),
